...
from slate.models.layers_dysat import StructuralAttentionLayer, TemporalAttentionLayer
...
class DySat(nn.Module):
    def __init__(
        ...
        assert aggr.lower() in [
            "mean",
            "sum",
            "max",
            "last",
        ], "Aggregation must be in [mean, sum, max, last]"
    ...
    def forward(self, graphs):
        ...
        for t in range(len(graphs)):
            ...
            if self.undirected:
                if graphs[t].edge_index.device != self.device:
                    graphs[t].edge_index = to_undirected(graphs[t].edge_index).to(
                        self.device
                    )
            else:
                if graphs[t].edge_index.device != self.device:
        ...
        structural_outputs = [
            g.x[:, None, :] for g in structural_out
        ]  # list of [Ni, 1, F]
