...
class EdgeAggregation(nn.Module):
    def __init__(self, aggr="mean") -> None:
        super().__init__()
        self.aggr = aggr
...
class NodeAggregation(nn.Module):
    ...
    def forward(
        self, final_emb: torch.Tensor, node: torch.LongTensor, time: torch.LongTensor
    ):
        """
        Arguments:
    ...
    def forward(self, x: torch.Tensor, pos: int) -> torch.Tensor:
        ...
        if self.learn:
            ...
            if self.linear:
                ...
                pe_temp = F.interpolate(
                    pe_temp, size=self.dim_te, mode="linear", align_corners=True
                )
        pe_temp = pe_temp.repeat(
            n, 1
        )  # Node share the same temporal encoding because they are in the same snapshot
        # pe_temp = self.dropout(pe_temp)
        return pe_temp


class LinkPredScore(nn.Module):
    def __init__(self, dim_emb: int, dropout: float = 0.1, edge: bool = False) -> None:
        """
        Arguments:
