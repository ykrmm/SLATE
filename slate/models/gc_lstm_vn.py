...
class GCLSTM_VN(torch.nn.Module):
    ...
    def add_virtual_node(self,edge_index):
        n = self.num_nodes -1
        node_snapshot = torch.unique(edge_index)
        node_snapshot = node_snapshot
        src = torch.ones(len(node_snapshot),dtype=torch.long) * n
        src = src.to(self.device)
        node_snapshot = node_snapshot.to(self.device)
        vn_co = torch.vstack((src,node_snapshot))
        edge_index = torch.hstack((edge_index,vn_co))
        return edge_index
    ...
    def forward(self,graphs):
        ...
        for t in range(len(graphs)):
            ...
            edge_index = self.add_virtual_node(edge_index)
            ...
            H,C = self.forward_snapshot(x, edge_index, edge_weight,H,C)
            output.append(H)
