...
def graphs_to_supra(
    ...
    total_nodes = (
        num_nodes * num_graphs + num_graphs if add_vn else num_nodes * num_graphs
    )
    if remove_isolated:
    ...
    edge_weight[edge_weight > 1] = (
        1  # Keep 1 everywhere except for the temporal connections
    )
