...
class EngineLinkPred(EngineBase):
    ...
    def train(self, datasets, T_train):
        ...
        for t in range(t_min, T_train):
            ...
            for feed_dict in dataloader_t:
                ...
                with torch.autocast(
                    ...
                    loss, pos_prob, neg_prob = self.model.get_loss_link_pred(
                        feed_dict, graphs_t
                    )  # [B,C] B: batch size, C: number of classes
                ...
                if self.log_train:
                    ...
                    labels = torch.cat(
                        (
                            torch.ones_like(pos_prob, device=self.device),
                            torch.zeros_like(neg_prob, device=self.device),
                        )
                    )
        ...
        if self.log_train:
            ...
            scores = {
                name: score.item() for name, score in zip(self.metrics.keys(), scores)
            }
            self.reset_metrics()
        else:
            scores = None
        ...
        for t in range(t_min, t_max):
            ...
            for feed_dict in dataloader_t:
                ...
                if self.use_val:
                    ...
                    roc, ap = (
                        roc_auc_score(labels.cpu().numpy(), pred.cpu().numpy()),
    ...
    def run(self):
        ...
        self.logger.info(
            "Total number of snapshots: {}".format(self.config.dataset.timestamp)
        )
        self.logger.info("Split protocol : {}".format(self.config.task.split))
        self.logger.info(
            "Edge sampling in evaluation: {}".format(self.config.task.sampling)
        )
        self.logger.info("Evolving graph: {}".format(self.config.model.evolving))
        self.logger.info(
            "Predicting next links: {}".format(self.config.model.pred_next)
        )
        self.logger.info(
        ...
        for i in range(n_runs):
            ...
            et = time.time()
            self.logger.info(
                "Time to construct the Discrete Dynamic Graph: {:.4f} s".format(et - st)
            )
            self.logger.info(
                "Number of edges in train: {}".format(len(self.dts.edge_index_train[0]))
            )
            self.logger.info(
                "Number of edges in val: {}".format(len(self.dts.edge_index_val[0]))
            )
            self.logger.info(
                "Number of edges in test: {}".format(len(self.dts.edge_index_test[0]))
            )
            ...
            try:
                self.model.set_device(
                    self.device
                )  # for models that need to know the device
            except Exception:
                pass
            ...
            if self.config.model.name == "EdgeBank":
                tw = t_test - t_val
                self.model.construct_node_history(
                    train_dts.get_dataset_t(t_train - 1).get_graphs(), tw
                )
            ...
            for ep in range(epoch):
                ...
                if (
                    self.config.model.name != "EdgeBank"
                ):  # EdgeBank do not need to be trained
                    et = time.time()
                ...
                if ep % eval_every == 0:
                    ...
                    if self.use_val:
                        ...
                        if (
                            ...
                            >= self.config.task.engine.early_stopping
                        ):  # No early stopping for the first run
            ...
            if self.use_val:
                ...
                self.logger.info(
                    "AP on test: {}".format(test_ap[val_ap.index(max(val_ap))])
                )  #
        ...
        self.logger.info(
            f"Maximum memory allocated: {torch.cuda.max_memory_allocated()} bytes"
        )
