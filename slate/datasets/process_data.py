...
def split_train_test(datadir, dataname, train_ratio: float = 0.8):
    ...
    if isfile(join(datadir, dataname, "train_classif.npy")) and isfile(
        join(datadir, dataname, "test_classif.npy")
    ):
        print(
            "Found existing train and test files for classification in",
            join(datadir, dataname),
        )
        return

    elif isfile(join(datadir, dataname, "train_classif.txt")) and isfile(
        join(datadir, dataname, "test_classif.txt")
    ):
        print(
            "Founding existing .txt files for classification in",
            join(datadir, dataname),
        )
        ...
        np.save(
            join(datadir, dataname, "train_classif.npy"),
            (np.array(train_nodes_all), np.array(train_labels_all)),
        )
        ...
        np.save(
            join(datadir, dataname, "test_classif.npy"),
            (np.array(test_nodes_all), np.array(test_labels_all)),
        )

        print(
            "Successfully created numpy train and test files for classification in",
            join(datadir, dataname),
        )
    ...
    for n, li in name_to_label.items():
        if li is not np.nan:
            unique_label.add(li)

    if isfile(
        join(datadir, "label_to_id.pkl")
    ):  # To share the same label id between all the datasets
        with open(join(datadir, "label_to_id.pkl"), "rb") as f:
            print("loading existing label_to_id.pkl")
            label_to_id = pickle.load(f)
    else:
        label_to_id = dict()
        for i, li in enumerate(unique_label):
            label_to_id[li] = i
    ...
    for i, l_id in enumerate(unique_label_dts):
        if l_id != -1 and count[i] > 5:  # at least 5 nodes for each label
            ...
            train_nodes = np.random.choice(
                np.array(node_label)[node_label[:, 1] == l_id][:, 0],
                nb_train,
                replace=False,
            )
            train_labels = np.array([l_id] * nb_train)
            test_nodes = np.array(node_label)[node_label[:, 1] == l_id][:, 0][
                ~np.isin(
                    np.array(node_label)[node_label[:, 1] == l_id][:, 0], train_nodes
                )
            ]
    ...
    np.save(
        join(datadir, dataname, "train_classif.npy"),
        (train_nodes_all, train_labels_all),
    )
    np.save(
        join(datadir, dataname, "test_classif.npy"), (test_nodes_all, test_labels_all)
    )
    ...
    print(
        "train and test successfully saved in ",
        join(datadir, dataname, "train_classif.npy"),
        " and test_classif.npy",
    )
